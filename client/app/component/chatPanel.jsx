  ...
  return (
    <div className="flex flex-col bg-primaryBg rounded p-5 w-full">
      <div className="flex w-full justify-end p-3 bg-primary gap-6 items-center rounded-t">
        <img
          src={props.side === "customer" ? "/user.png" : props.chat ? props.chat.user.profile : "/user.png"}
          alt="User"
          className="w-20 h-20 rounded-full"
        />
        <h1 className="text-white text-3xl font-bold">
          {props.side === "customer" ? "Admin" : props.chat ? props.chat.user.name : ""}
        </h1>
      </div>
      <div className="flex flex-col bg-white h-screen gap-1 overflow-scroll overflow-x-hidden">
        {props.chat?.messages.map((message) => {
          return (
            <Message message={message} side={props.side} key={message.id} id={props.side === "services" ? "" : props.id}/>
          );
        })}
